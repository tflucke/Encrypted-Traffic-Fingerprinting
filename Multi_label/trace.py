...
class Trace():
	...
	def load_pcap(self, pathname, labels):
		self.labels = labels
	...
	def construct_trace(self, packets, times, labels):
	...
	def get_windowed(self, window_size = 1024):
		...
		for x in range(0,(self.num_packets/window_size)):
			...
			temp.construct_trace(packets[x*window_size:(x+1)*window_size], times[x*window_size:(x+1)*window_size], self.labels)
